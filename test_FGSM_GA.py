...
def get_args():
    ...
    parser.add_argument('--model_flag', default='best', type=str, help='model name')
...
if args.model_flag=='best':
    try:
        target_model.load_state_dict(checkpoint['best'])
    except:
    ...
    try:
        target_model.load_state_dict(checkpoint['last'])
    except:
        new_state_dict = OrderedDict()
        for k, v in checkpoint.items():
            name = k[7:]  # remove `module.`
